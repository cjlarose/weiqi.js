...
function createBoard(size, stones) {
  ...
  var Board = {
    ...
    toArray: function() {
      return mori.toJs(mori.map(function(i) {
        return mori.map(function(j) {
    ...
    getStones: function(color) {
      ...
      var colorMap = mori.zipmap(positions, mori.map(this.getStone, positions));
      var correctColor = function(pair) { return mori.nth(pair, 1) == color; };
      var stones = mori.pipeline(colorMap,
        mori.partial(mori.filter, correctColor),
        mori.partial(mori.map, mori.first));
      return mori.toJs(stones);
    },
