...
var BoardPrototype = {
  ...
  inBounds: function(coords) {
    return inBounds(this.size, coords);
  },

  getStone: function(coords) {
    return getStone(this.intersections, coords);
  },

  toString: function() {
    return _.map(mori.clj_to_js(this.intersections), function(row) {return row.join('');}).join('\n');
  },
  ...
  play: function(color, coords) {
    ...
    var newBoard = replaceStone(this.intersections, coords, color);
    ...
    var captured = _(neighbors)
      ...
      .map(_.first)
      .map(getGroup.bind(this, newBoard, this.size))
