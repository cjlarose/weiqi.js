  ...
  var Board = {
    ...
    areaScore: function() {
      ...
      mori.each(positions, function(coords) {
        ...
        var surroundingColors = mori.into(mori.set(),
          mori.map(function(pair) { return mori.nth(pair, 1);},
                   mori.get(group, 'surrounding')));

        if (state == Constants.EMPTY && mori.count(surroundingColors) === 1) {
